    ...
    pe4=[];
        ...
        for numiter=1:5
            ...
            if option_modulation==1 || option_modulation==3
                ...
                %BER of message with no codification (BPSK)
            ...
            if option_modulation==2 || option_modulation==3
                ...
                %BER of message with no codification (QPSK)
            ...
            %Variables para dividir mensaje en paquetes
            % mensaje+bits paridad
            tamanio=size(bitsr2,2);
            div=1; %mensaje+bits paridad

            bitsr=[]; %matriz decodificada solo mensaje
            bitsrq=[]; %matriz decodificada solo mensaje QPSK

            while(div<tamanio)

                %Divisi?n de mensaje recibido en paquetes
                ...
                %Obtenci?n de mensaje decodificado
                b=bits2(4:7);
                bitsr=[bitsr b];
            ...
            %----- BER con codificacion Hamming 7,4 QPSK
            error2q=error2q+sum(xor(bitsc1,bitsrq));
            pe2q=[pe2q sum(xor(bitsc1,bitsrq))/N];

            %----- BER con codificacion Hamming 7,4 BPSK
            ...
            if option_modulation==2 || option_modulation==3
                ...
                bits4=viterbi_s(g,1,y4);    %No demodulation

                %BER of soft decision (QPSK)
...
switch option_modulation
    case 1
        ...
        ylim([10^(-4) 10^(-1)]);
        hleg = legend('AWGN','Hamming','Hard','Soft');
    ...
    case 3
        ...
        hold on;
        semilogy(ebn0db,pet4,'--mx');
        xlabel('Eb/N0, dB')
        ylabel('Bit Error Rate')
        title('BER Curves (QPSK and BPSK)')
        ylim([10^(-4) 10^(-1)]);
        hleg = legend('AWGN (BPSK)','Hamming (BPSK)','Hard (BPSK)', 'Soft (BPSK)','AWGN (QPSK)','Hamming (QPSK)','Hard (QPSK)', 'Soft (QPSK)');
        set(hleg,'Location','EastOutside')
        grid on;
end
